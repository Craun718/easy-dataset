 ...
 * @param {string[]} options.fileIds - 按文件ID筛选
 * @param {string} options.keyword - 按名称关键词搜索
 * @returns {Promise<Array>} - 文本块列表
 */
export async function getChunksForExport(projectId, { fileIds, keyword, offset, limit } = {}) {
  try {
    ...
    const result = await db.chunks.findMany({
      where,
      select: {
        name: true,
        fileName: true,
    ...
    const sorted = sortByName(result);

    // 支持分页：排序后再切片，保证分页边界与自然排序一致
    const hasPaging = offset != null || limit != null;
    if (hasPaging) {
    ...
    console.error('Failed to get chunks for export:', error);
    throw error;
  }
}

/**
 * 获取或创建 UploadFile 记录（用于导入时关联 fileId）
 * @param {string} projectId - 项目ID
 * @param {string} fileName - 文件名
 * @returns {Promise<Object>} - UploadFile 记录
 */
export async function getOrCreateUploadFile(projectId, fileName) {
  try {
    let file = await db.uploadFiles.findFirst({
      where: { projectId, fileName }
    });

    if (!file) {
      file = await db.uploadFiles.create({
        data: {
          projectId,
          fileName,
          fileExt: '.md',
          path: '',
          size: 0,
          md5: `imported-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
        }
      });
    }
    ...
    console.error('Failed to get or create upload file:', error);
    throw error;
  }
}
